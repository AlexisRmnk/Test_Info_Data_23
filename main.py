...
def crear_tabla():
    cursor = connection.cursor() #nota: crea una instancia de conexion a la BD
    query = ("CREATE TABLE customers(customerid INT PRIMARY KEY,"
        " name VARCHAR(50), occupation VARCHAR(50), email VARCHAR(50),"
        " company VARCHAR(50), phonenumber VARCHAR(20), age INT)")
    try:
    ...
    cursor.close() # cierra la conexion de la instancia
...
def insertar_datos():
    cursor = connection.cursor()
    query = "INSERT INTO customers(customerid, name, occupation, email, company, phonenumber, age) VALUES (%s, %s, %s, %s, %s, %s, %s)" # usa valores posicionales ( https://www.psycopg.org/docs/usage.html#passing-parameters-to-sql-queries )
    try:
